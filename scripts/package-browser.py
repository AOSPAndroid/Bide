...
with ZipFile(target, 'w', ZIP_DEFLATED, compresslevel=6) as archive:
    ...
    archive.writestr('source/REBUILD.md', 'Run Install Dependencies.bat to use your installed Node.js and npm, restore the Office assets from ../site, install the locked npm dependencies, and build the editor. This source rebuild may need internet; the prebuilt root launchers need no setup downloads. Then run launch bide.bat. For manual builds, run npm ci --ignore-scripts, npm run assets:office, and npm run build. npm test runs browser PDF engine tests; the Python test file is the older engine reference.\n')
