...
import {validatePsdHeader,requiresComposite} from '../tmp/psd-core.mjs';
...
test('Photoshop features outside the supported raster subset use the composite preview',async()=>{
 for(const extra of [{children:[]},{mask:{}},{effects:{}},{adjustment:{}},{placedLayer:{}},{clipping:true},{blendMode:'dissolve'}])assert.equal(requiresComposite({width:100,height:100,children:[extra]}),true);
 const grouped={...sample,children:[{name:'Group',children:sample.children}]};await writeFile('tmp/psd/grouped.psd',new Uint8Array(writePsd(grouped)));
});
