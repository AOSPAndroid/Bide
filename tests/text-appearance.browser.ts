...
try {
 ...
 Object.assign(text,{id:'test-text',name:'Test text',pdfOriginalFont:'Arial',recognizedKey:'test-region'});c.add(text);c.renderAll();
 ...
 const snapshot=c.toObject();const saved=text.toObject() as any;if(saved.id!=='test-text'||saved.name!=='Test text'||saved.recognizedKey!=='test-region'||saved.objectCaching!==false)throw Error('Text metadata not retained');await c.loadFromJSON(snapshot);c.renderAll();const restored=c.getObjects()[0] as Textbox;
 if(!(restored instanceof Textbox)||restored.textSoftness!==.6)throw Error('Softness not restored');
 const after=edge();if(after.partial!==soft.partial)throw Error('Reload changed pixels: '+JSON.stringify({soft,after}));
