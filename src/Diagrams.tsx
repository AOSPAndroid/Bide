...
export default function Diagrams({active, commands, onPlace, onNew, onPalette}: Props) {
  ...
  return <section className="diagrams-workspace" style={{display:active ? 'flex' : 'none'}} aria-label="Diagram workspace">
    ...
    <div className="diagram-header">
      ...
      <div className="diagram-actions"><button onClick={()=>setTemplates(true)} disabled={!ready||!!busy}><FilePlus2 size={14}/>New diagram</button><button onClick={()=>fileInput.current?.click()} disabled={!ready||!!busy}><FolderOpen size={14}/>Open diagram</button><button onClick={()=>request('save')} disabled={!ready||!!busy}><Save size={14}/>Save .drawio</button><span className="divider"/><button onClick={()=>request('svg')} disabled={!ready||!!busy}>Export SVG</button><button onClick={()=>request('png')} disabled={!ready||!!busy}>Export PNG</button><button onClick={()=>request('pdf')} disabled={!ready||!!busy}><Download size={14}/>Export PDF</button><button className="primary" onClick={()=>request('place')} disabled={!ready||!!busy}><ImagePlus size={14}/>Place in document</button></div>
    </div>
