...
export async function svgToPdf(svg:string,width:number,height:number,embedded:Record<string,string>={}):Promise<Uint8Array<ArrayBuffer>> {
 ...
 return new Promise((resolve,reject)=>{
  ...
  for(const [name,data]of Object.entries(embedded))doc.registerFont(name,Uint8Array.from(atob(data),c=>c.charCodeAt(0)) as any);
 ...
 const name=family.split(',')[0].trim().replace(/^['"]|['"]$/g,'');const item=selected.find(font=>font.family===name);if(embedded[name]){options.fauxBold=!!bold;options.fauxItalic=!!italic;return name;}
