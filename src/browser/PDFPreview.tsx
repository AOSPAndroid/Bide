...
export default function PDFPreview({source,index,scale,className,alt='',lazy=false}:{source:string;index:number;scale:number;className?:string;alt?:string;lazy?:boolean}){
 const [src,setSrc]=useState(''),[error,setError]=useState(''),[visible,setVisible]=useState(!lazy);
 ...
 useEffect(()=>{if(!visible)return;let active=true;setSrc('');setError('');pagePreview(source,index,scale).then(url=>{if(active)setSrc(url);}).catch(e=>{if(active)setError(e.message);});return()=>{active=false;};},[source,index,scale,visible]);
