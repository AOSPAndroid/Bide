...
export async function recognizeImage(image:HTMLCanvasElement):Promise<TextRegion[]>{
 ...
 for(const block of data.blocks||[])for(const paragraph of block.paragraphs)for(const line of paragraph.lines){if(!line.text.trim()||line.confidence<35)continue;const b=line.bbox,w=b.x1-b.x0,h=b.y1-b.y0;if(w<1||h<1)continue;const rgb=ctx.getImageData(Math.max(0,b.x0-2),Math.max(0,b.y0-2),1,1).data;const background='#'+[...rgb.slice(0,3)].map(v=>v.toString(16).padStart(2,'0')).join('');const light=rgb[0]+rgb[1]+rgb[2]>384;regions.push({id:'ocr-'+regions.length,text:line.text.trim(),left:b.x0,top:b.y0,width:w,height:h,size:h/.75,baseline:b.y1,fontName:'Image text (font unknown)',bold:false,italic:false,color:light?'#151515':'#ffffff',kind:'ocr',confidence:line.confidence,background});}
