...
export default function App() {
  const [view, setView] = useState<'editor'|'diagrams'>('editor');
  ...
  function fit() {
    ...
    if (root) { zoomAnchor.current = null; setZoom(fitZoom(p.width, p.height, root.clientWidth, root.clientHeight)); root.scrollTo(0,0); }
  }
  ...
  function openDiagrams() {diagramTarget.current=undefined;setDiagramsVisited(true);setView('diagrams');setMenu(null);}
  async function placeDiagram(data: string, xml: string, name: string) {
    ...
    diagramTarget.current = undefined; setView('editor'); notify('Diagram placed. Its editable source is included when you save a .bide project.');
  }
  ...
  useEffect(() => {
    ...
    c.on('path:created', e => { Object.assign(e.path, { id: uid(), name: toolRef.current==='highlight'?'Freehand highlight':'Brush stroke' }); saveCanvasRef.current(); });
    ...
    c.on('mouse:down', e => {
      ...
      if (['select','brush','highlight'].includes(t)) return;
  ...
  useEffect(() => {
    ...
    c.loadFromJSON(page.canvas, undefined, { signal: controller.signal }).then(() => {
      ...
      if (fittedPage.current !== page.id) {fittedPage.current=page.id; fit();}
  ...
  useEffect(() => {
    ...
    if (c.isDrawingMode) {
      const brush = new PencilBrush(c); brush.color = tool==='highlight'?markerColor(highlightColor,highlightOpacity):color;
      brush.width = tool==='highlight'?highlightSize:brushSize; brush.strokeLineCap='round';brush.strokeLineJoin='round';
      c.freeDrawingBrush = brush;
    }
    if (tool !== 'select') c.discardActiveObject();
    c.requestRenderAll();
  }, [tool, color, brushSize, highlightColor, highlightSize, highlightOpacity, ready]);
  ...
  const diagramActions = [
    {label:'Back to design & PDF',hint:'',icon:FileText,run:()=>setView('editor')},
  ...
  }, [modal,palette,busy,page,selected,view]);
  ...
  return <div className="app" onDragOver={e=>{ if(view==='editor'&&e.dataTransfer.types.includes('Files')) { e.preventDefault(); setDraggingFile(true); } }} onDrop={e=>{ if(view==='editor'&&e.dataTransfer.files.length) { e.preventDefault(); setDraggingFile(false); void importFiles([...e.dataTransfer.files],true); } }}>
    ...
    <div className="editor-shell" style={{display:view==='editor'?'flex':'none'}}>
