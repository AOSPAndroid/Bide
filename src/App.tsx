import type {OcrGrouping} from './ocr-segmentation';
import {repairTextBackground} from './text-replacement';
import {Textbox} from './text-appearance';
import {unlockPdfPage} from './pdf-layer';
import {thumbnailScale,editorPreviewScale} from './render-budget';
import { useEffect, useLayoutEffect, useRef, useState, type ReactNode } from 'react';
import { Canvas, StaticCanvas, Rect, Ellipse, Line, FabricImage, FabricObject, PencilBrush, Path, Group, ActiveSelection, filters } from 'fabric';
...
import {fallbackFont,pdfFallbackStyle,fitOcrText,type TextRegion} from './text-regions';
...
type Obj = FabricObject & { textSoftness?:number; pdfFontData?:string;pdfOriginalFont?:string;pdfFontFallback?:string;pdfFontBold?:boolean;pdfFontItalic?:boolean;recognizedKey?:string; id?: string; name?: string; diagramXml?: string; text?: string; fontSize?: number; fontFamily?: string; fontWeight?: string | number; fontStyle?: string; underline?: boolean; textAlign?: string; isEditing?: boolean };
...
export default function App() {
  ...
  const [ocrStyle,setOcrStyle]=useState('regular');
  const [ocrGrouping,setOcrGrouping]=useState<OcrGrouping>('smart');
  const [replacementBackground,setReplacementBackground]=useState<'repair'|'sampled'|'solid'|'transparent'>('sampled'),[replacementColor,setReplacementColor]=useState('#ffffff');
  const [replacementPadding,setReplacementPadding]=useState(-1);
  ...
  function openLayerMenu(o:Obj,x:number,y:number){
    const c=canvas.current;if(!c)return;
    const active=c.getActiveObject();if(active instanceof Textbox&&active.isEditing)active.exitEditing();
    c.setActiveObject(o);c.isDrawingMode=false;c.requestRenderAll();syncSelection();setTool('select');
    setLayerMenu({id:o.id!,x:Math.max(8,Math.min(x,innerWidth-260)),y:Math.max(8,Math.min(y,innerHeight-Math.min(540,innerHeight-16)))});
  }
  ...
  useEffect(() => {
    ...
    c.on('contextmenu', e=>{
      if(loading.current)return;const skip=c.skipTargetFind;c.skipTargetFind=false;
      let target;try{target=c.findTarget(e.e as MouseEvent).target;}finally{c.skipTargetFind=skip;}
      if(target)openLayerMenu(target as Obj,(e.e as MouseEvent).clientX,(e.e as MouseEvent).clientY);else setLayerMenu(null);
    });
    c.on('mouse:down', e => {
  ...
  useEffect(()=>{
    ...
    const scan=async()=>{setTextScanStatus('Detecting text…');try{
      ...
      if(cancelled)return;const ratio=Math.min(1,2400/Math.max(source.width,source.height));const reduced=document.createElement('canvas');reduced.width=Math.round(source.width*ratio);reduced.height=Math.round(source.height*ratio);reduced.getContext('2d')!.fillStyle=p.color==='transparent'?'#ffffff':p.color;reduced.getContext('2d')!.fillRect(0,0,reduced.width,reduced.height);reduced.getContext('2d')!.drawImage(source,0,0,reduced.width,reduced.height);
      const {recognizeImage}=await import('./browser/ocr');const detected=await recognizeImage(reduced,scanController.signal,ocrGrouping);if(cancelled)return;const sx=p.width/reduced.width,sy=p.height/reduced.height;const regions=detected.map(r=>({...r,left:r.left*sx,top:r.top*sy,width:r.width*sx,height:r.height*sy,size:r.size*sy,baseline:r.baseline*sy})).filter(r=>!c.getObjects().some(o=>{const key=(o as Obj).recognizedKey;if(key===recognitionKey(r))return true;if(!key?.startsWith('ocr:'))return false;const b=o.getBoundingRect(),x=r.left+r.width/2,y=r.top+r.height/2;return x>=b.left&&x<=b.left+b.width&&y>=b.top&&y<=b.top+b.height;}));
  ...
  },[ready,initializing,page.id,page.source,textScanRevision,ocrGrouping]);
  ...
  async function editDetectedText(region:TextRegion){
    ...
    try{if(region.kind==='ocr'&&replacementBackground==='transparent'&&originalPage.source)throw new Error('Unlock Original PDF first to clear scanned text to transparency.');let family=region.kind==='ocr'?ocrFont:fallbackFont(region.fontName),embedded=false;if(region.fontData&&region.fontId){try{await ensureEmbeddedFont(region.fontId,region.fontData);family=region.fontId;embedded=true;}catch{}}await ensureFont(family);
      const bold=region.kind==='ocr'?ocrStyle.includes('bold'):region.bold,italic=region.kind==='ocr'?ocrStyle.includes('italic'):region.italic;
      let replacementSize=region.size,ocrAscent=region.height;
      if(region.kind==='ocr'){const ctx=document.createElement('canvas').getContext('2d')!;ctx.font=`${italic?'italic ':''}${bold?'bold ':''}100px "${family}"`;const fit=fitOcrText(region,ctx.measureText(region.text));replacementSize=fit.size;ocrAscent=fit.ascent;}
      const text=Object.assign(new Textbox(region.text,{left:region.left,top:region.top,width:Math.max(region.width+4,20),fontSize:replacementSize,fontFamily:family,fontWeight:embedded?'normal':bold?'bold':'normal',fontStyle:embedded?'normal':italic?'italic':'normal',fill:region.color,lineHeight:1,splitByGrapheme:true,objectCaching:false}),{id:uid(),name:region.text.slice(0,40),pdfOriginalFont:region.fontName,pdfFontData:embedded?region.fontData:undefined,pdfFontFallback:fallbackFont(region.fontName),pdfFontBold:bold,pdfFontItalic:italic,recognizedKey:recognitionKey(region)});
      ...
      if(region.kind==='ocr'){
        const padding=replacementPadding<0?Math.max(2,region.height*.1):replacementPadding,left=Math.max(0,region.left-padding),top=Math.max(0,region.top-padding),width=Math.min(originalPage.width,region.left+region.width+padding)-left,height=Math.min(originalPage.height,region.top+region.height+padding)-top;
        if(replacementBackground==='transparent'){
          const c=canvas.current!;const images=c.getObjects().filter(o=>o instanceof FabricImage&&o.visible&&(()=>{const b=o.getBoundingRect();return b.left<left+width&&b.top<top+height&&b.left+b.width>left&&b.top+b.height>top;})());
          if(!images.length)throw new Error('Select image text to clear its background.');
          if(images.some(o=>o.lockMovementX))throw new Error('Unlock the image layers beneath this text first.');
          for(const image of images){
            const blend=image.globalCompositeOperation;let raster:HTMLCanvasElement;try{image.globalCompositeOperation='source-over';raster=pageRaster(c,originalPage.width,originalPage.height,[image]);}finally{image.globalCompositeOperation=blend;}raster.getContext('2d')!.clearRect(left,top,width,height);
            const replacement=rasterLayer(raster,(image as Obj).name||'Image',(image as Obj).id!);const index=objects.findIndex(o=>(o as Obj).id===(image as Obj).id);
            if(index>=0){if(replacement){replacement.globalCompositeOperation=blend;objects[index]=replacement.toObject();}else objects.splice(index,1);}
          }
        }else if(replacementBackground==='repair'){
          let raster:HTMLCanvasElement;
          if(originalPage.source){const image=new Image();image.src=await pagePreview(originalPage.source,originalPage.index,1);await image.decode();raster=document.createElement('canvas');raster.width=Math.ceil(originalPage.width);raster.height=Math.ceil(originalPage.height);raster.getContext('2d')!.drawImage(image,0,0,raster.width,raster.height);}
          else raster=pageRaster(canvas.current!,originalPage.width,originalPage.height,canvas.current!.getObjects().filter(o=>o instanceof FabricImage));
          const repaired=repairTextBackground(raster.getContext('2d')!.getImageData(0,0,raster.width,raster.height),{left,top,width,height});
          const patchCanvas=document.createElement('canvas');patchCanvas.width=repaired.width;patchCanvas.height=repaired.height;const context=patchCanvas.getContext('2d')!,pixels=context.createImageData(repaired.width,repaired.height);pixels.data.set(repaired.data);context.putImageData(pixels,0,0);
          const patch=Object.assign(new FabricImage(patchCanvas,{left:repaired.left,top:repaired.top}),{id:uid(),name:'Repaired text background'});objects.push(patch.toObject());
        }else{
          const patch=Object.assign(new Rect({left,top,width,height,fill:replacementBackground==='solid'?replacementColor:region.background||'#ffffff',strokeWidth:0}),{id:uid(),name:'Text background patch'});objects.push(patch.toObject());
        }
      }
      if(pageIdRef.current!==originalPage.id)return;
      objects.push(text.toObject());pendingTextEdit.current=text.id;setTool('select');setTextRegions([]);commit({...current,sources:nextData&&nextSource?{...current.sources,[nextSource]:nextData}:current.sources,pages:current.pages.map(p=>p.id===originalPage.id?{...p,source:nextSource,color:region.kind==='ocr'&&replacementBackground==='transparent'?'transparent':p.color,canvas:{...p.canvas,objects},thumb:undefined}:p)},true);
      notify(region.kind==='ocr'?(replacementBackground==='transparent'?'Old image text cleared to transparency. Undo restores the cleared area.':'OCR text is editable. Check recognition, font and background patch.'):embedded?'Editing with the embedded PDF font.':'Text is editable using a matching bundled font.');
    }catch(e){fail(e);}finally{setBusy('');}
  }
  ...
  function updateObject(values: Record<string, unknown>, save = true) {
    ...
    if(o instanceof Textbox&&o.isEditing)o.exitEditing();
  ...
  async function changeTextFont(family:string) {
    ...
    }catch(error){fail(error instanceof Error?error:new Error('Could not load this font.'));}
  }
  ...
  async function payload() {
    ...
    for (const p of current.pages) {
      ...
      try {
        ...
        for(const object of c.getObjects()){const o=object as Obj;if(o instanceof Textbox&&(o as Obj).pdfFontData&&o.fontFamily.startsWith('PDFfont-')&&!await fontSupportsText((o as Obj).pdfFontData!,o.text)){const family=(o as Obj).pdfFontFallback||'Lato';await ensureFont(family);o.set({fontFamily:family,...pdfFallbackStyle(o as Obj)});o.initDimensions();}}
  ...
  async function exportTransparentPng(textOnly=false){
    const c=canvas.current;if(!c)return;const object=c.getActiveObject();
    if(textOnly&&!(object instanceof Textbox))return;
    if(!textOnly&&page.source){notify('Unlock Original PDF first to export its page with transparency.');return;}
    setBusy('Exporting transparent PNG…');
    try{
      let output:HTMLCanvasElement;
      if(textOnly){const b=object!.getBoundingRect();output=object!.toCanvasElement({multiplier:Math.min(3,8192/Math.max(1,b.width,b.height),Math.sqrt(16000000/(Math.max(1,b.width)*Math.max(1,b.height))))});}
      else{const old=c.backgroundColor;try{c.backgroundColor='';output=c.toCanvasElement(Math.min(3,8192/Math.max(page.width,page.height),Math.sqrt(16000000/(page.width*page.height)))/c.getZoom());}finally{c.backgroundColor=old;}}
      const blob=await new Promise<Blob>((resolve,reject)=>output.toBlob(b=>b?resolve(b):reject(new Error('PNG export failed.')),'image/png'));
      download(blob,`${safeName(textOnly?(object as Obj).name||'text':project.name)}-transparent.png`);notify('Transparent PNG exported.');
    }catch(e){fail(e);}finally{setBusy('');}
  }
  ...
  const requireLayer=!selected?'Select a layer first':selected.lockMovementX&&!(selected instanceof Textbox&&selected.isEditing)?'Unlock the layer first':undefined;
  ...
  const moreActions:PaletteAction[] = [
    {label:'Export text as transparent PNG',hint:'',icon:Download,disabled:requireText,run:()=>void exportTransparentPng(true)},
    {label:'Export page as transparent PNG',hint:'',icon:Download,disabled:page.source?'Unlock Original PDF first':undefined,run:()=>void exportTransparentPng()},
    {label:'Choose OCR selection grouping',hint:'',keywords:'teams avatar initials split words lines',icon:Type,run:()=>focusSetting('OCR selection grouping')},
    {label:'Choose OCR replacement font',hint:'',keywords:'teams segoe aptos calibri corporate',icon:Type,run:()=>focusSetting('OCR replacement font')},
    {label:'Choose OCR replacement weight',hint:'',keywords:'bold regular italic text',icon:Bold,run:()=>focusSetting('OCR replacement weight')},
    {label:'Adjust replacement background',hint:'',icon:SlidersHorizontal,run:()=>focusSetting('Text replacement background')},
    {label:'Adjust text cleanup padding',hint:'',icon:SlidersHorizontal,run:()=>focusSetting('Text cleanup padding')},
    {label:'Adjust letter spacing',hint:'',icon:SlidersHorizontal,disabled:requireText,run:()=>focusSetting('Letter spacing')},
    {label:'Adjust text softness',hint:'',keywords:'blur sharpness scan match',icon:SlidersHorizontal,disabled:requireText,run:()=>focusSetting('Text softness')},
  ...
  return <div className={`app workspace-${view}`} onDragOver={e=>{ if(view!=='diagrams'&&e.dataTransfer.types.includes('Files')) { e.preventDefault(); setDraggingFile(true); } }} onDrop={e=>{ if(view!=='diagrams'&&e.dataTransfer.files.length) { e.preventDefault(); setDraggingFile(false); setPdfHome(false);void importFiles([...e.dataTransfer.files],true); } }}>
    ...
    {selected instanceof Rect&&<div className="property-section"><Field label="Corner radius" min={0} max={Math.min(selected.width,selected.height)/2} value={selected.rx} onChange={v=>updateObject({rx:v,ry:v})}/></div>}{selectedText && <div className="property-section"><div className="section-label">TEXT</div>{systemFonts.includes(selected.fontFamily)&&<p className="fine-print">System fonts use this PC’s installation. Workplace font text is rasterized on PDF/SVG export to preserve its appearance; save .bide to keep it editable.</p>}{(selected as Obj).pdfOriginalFont&&<p className="fine-print">{selected.fontFamily.startsWith('PDFfont-')?'Original embedded font: ':'Suggested font for: '}{(selected as Obj).pdfOriginalFont}</p>}<label className="stacked-label">Text softness · {((selected as Obj).textSoftness||0).toFixed(2)} px<input aria-label="Text softness" type="range" min="0" max="2" step="0.05" value={(selected as Obj).textSoftness||0} onChange={e=>updateObject({textSoftness:Number(e.target.value),objectCaching:false})}/></label><div className="selection-actions"><button onClick={()=>updateObject({textSoftness:0,objectCaching:false})}>Crisp</button><button onClick={()=>updateObject({textSoftness:.35,objectCaching:false})}>Slightly soft</button></div><p className="fine-print">Match scanned text by adjusting softness and fill color. Softened text exports as an image; save .bide to keep it editable.</p><label className="color-field"><input type="color" aria-label="Text box background color" value={selected.textBackgroundColor&&/^#[a-f\d]{6}$/i.test(selected.textBackgroundColor)?selected.textBackgroundColor:'#ffffff'} onChange={e=>updateObject({textBackgroundColor:e.target.value})}/><span>Text background</span><button onClick={()=>updateObject({textBackgroundColor:''})}>Transparent</button></label><button onClick={()=>void exportTransparentPng(true)}>Export text as transparent PNG</button><textarea aria-label="Text content" value={selected.text||''} onChange={e=>updateObject({text:e.target.value})}/><div className="field-grid"><select aria-label="Text alignment" value={selected.textAlign} onChange={e=>updateObject({textAlign:e.target.value})}>{['left','center','right','justify'].map(x=><option key={x}>{x}</option>)}</select><Field label="Letter spacing" value={(selected as Textbox).charSpacing||0} min={-200} max={1000} step={10} onChange={v=>updateObject({charSpacing:v})}/><Field label="Line height" value={(selected as Textbox).lineHeight} min={0.6} max={4} step={0.1} onChange={v=>updateObject({lineHeight:v})}/></div></div>}
    ...
    <div className="layer-retouch-actions"><button disabled={!selected} onClick={openWarp}>Warp</button><button disabled={!selected} onClick={mergeDown}>Merge Down</button>{selected&&<label>Opacity<input aria-label="Layer opacity" type="range" min={0} max={100} value={Math.round(selected.opacity*100)} onChange={e=>updateObject({opacity:Number(e.target.value)/100})}/><span>{Math.round(selected.opacity*100)}%</span></label>}</div><div className="layers-header"><span><Layers size={14}/>Layers <b>{layers.length+(page.source?1:0)}</b></span><div><IconButton icon={ArrowUp} label="Bring layer forward" onClick={()=>moveLayer('up')} disabled={!selected}/><IconButton icon={ArrowDown} label="Send layer backward" onClick={()=>moveLayer('down')} disabled={!selected}/><IconButton icon={Plus} label="Add text layer" onClick={addText}/></div></div><div className="layers-list">{layers.map(o=><div className={`layer-row ${selected===o || (selected instanceof ActiveSelection && selected.contains(o))?'selected':''}`} key={o.id} onContextMenu={e=>{e.preventDefault();openLayerMenu(o,e.clientX,e.clientY);}} onClick={()=>{canvas.current?.setActiveObject(o);canvas.current?.requestRenderAll();syncSelection();setTool('select');}}><button title={o.visible?'Hide layer':'Show layer'} aria-label={`${o.visible?'Hide':'Show'} ${o.name}`} onClick={e=>{e.stopPropagation();o.set({visible:!o.visible});canvas.current?.requestRenderAll();saveCanvas();}}>{o.visible?<Eye size={13}/>:<EyeOff size={13}/>}</button><span className="layer-type">{o instanceof Textbox?<Type size={13}/>:o instanceof FabricImage?<FileImage size={13}/>:<Square size={12}/>}</span><span className="layer-name">{o.name || o.type}</span><button aria-label={`${o.lockMovementX?'Unlock':'Lock'} ${o.name}`} title={o.lockMovementX?'Unlock layer':'Lock layer'} onClick={e=>{e.stopPropagation();toggleLock(o);}}>{o.lockMovementX?<LockKeyhole size={12}/>:<UnlockKeyhole size={12}/>}</button></div>)}{page.source&&<div className="layer-row background-layer"><Eye size={13}/><FileText size={13}/><span>Original PDF</span><button aria-label="Unlock Original PDF" title="Convert this page to an image layer for Marquee, Lasso and retouching. Original text and links become pixels; Undo restores the PDF." disabled={!!busy} onClick={()=>void unlockOriginalPdf()}><LockKeyhole size={12}/></button></div>}{!layers.length&&!page.source&&<div className="layers-empty">Add text, images, or shapes.<br/>Your layers will appear here.</div>}</div><div className="layer-footer"><span>{selected?'Layer selected':'No layer selected'}</span><IconButton icon={Trash2} label="Delete selected layer" onClick={removeSelected} disabled={!selected}/></div></aside></main>
    {layerMenu&&selected&&<><div className="layer-menu-dismiss" onClick={()=>setLayerMenu(null)} onContextMenu={e=>{e.preventDefault();setLayerMenu(null);}}/><div className="page-context-menu text-context-menu" role="menu" aria-label={selectedText?'Text box actions':'Layer actions'} style={{left:layerMenu.x,top:layerMenu.y,width:248,maxHeight:'min(540px, calc(100vh - 16px))',overflowY:'auto'}} onKeyDown={e=>{if(e.key==='Escape'){e.preventDefault();e.stopPropagation();setLayerMenu(null);}if(['ArrowDown','ArrowUp','Home','End'].includes(e.key)){e.preventDefault();e.stopPropagation();const buttons=Array.from(e.currentTarget.querySelectorAll<HTMLButtonElement>('button:not(:disabled)'));const current=buttons.indexOf(document.activeElement as HTMLButtonElement);buttons[e.key==='Home'?0:e.key==='End'?buttons.length-1:(current+(e.key==='ArrowDown'?1:-1)+buttons.length)%buttons.length]?.focus();}}}>
      <strong>{selectedText?'Text box':'Layer'} actions</strong>
      {selectedText&&<>
        <button role="menuitem" disabled={!!selected.lockMovementX} autoFocus onClick={()=>{setLayerMenu(null);(selected as Textbox).enterEditing();(selected as Textbox).selectAll();canvas.current?.requestRenderAll();}}>Edit text</button>
        {['Font family','Font size','Object fill','Text softness'].map(label=><button key={label} role="menuitem" disabled={!!selected.lockMovementX} onClick={()=>{setLayerMenu(null);focusSetting(label);}}>{label==='Object fill'?'Text color':label}</button>)}
        <button role="menuitem" disabled={!!selected.lockMovementX} onClick={()=>{updateObject({textBackgroundColor:''});setLayerMenu(null);}}>Transparent text background</button><button role="menuitem" onClick={()=>{setLayerMenu(null);void exportTransparentPng(true);}}>Export text as transparent PNG</button><button role="menuitem" disabled={!!selected.lockMovementX} onClick={()=>{updateObject({textSoftness:.35,objectCaching:false});setLayerMenu(null);}}>Slightly soft</button>
        <button role="menuitem" disabled={!!selected.lockMovementX} onClick={()=>{updateObject({textSoftness:0,objectCaching:false});setLayerMenu(null);}}>Crisp text</button>
        <button role="menuitem" disabled={!!selected.lockMovementX} onClick={()=>{updateObject({fontWeight:selected.fontWeight==='bold'?'normal':'bold'});setLayerMenu(null);}}>Toggle bold</button>
        <button role="menuitem" disabled={!!selected.lockMovementX} onClick={()=>{updateObject({fontStyle:selected.fontStyle==='italic'?'normal':'italic'});setLayerMenu(null);}}>Toggle italic</button>
        <button role="menuitem" disabled={!!selected.lockMovementX} onClick={()=>{updateObject({underline:!selected.underline});setLayerMenu(null);}}>Toggle underline</button>
      </>}
      <button role="menuitem" disabled={!!selected.lockMovementX} onClick={()=>{duplicate();setLayerMenu(null);}}>Duplicate · Ctrl D</button>
      <button role="menuitem" disabled={!!selected.lockMovementX} onClick={()=>{moveLayer('up');setLayerMenu(null);}}>Bring forward</button>
      <button role="menuitem" disabled={!!selected.lockMovementX} onClick={()=>{moveLayer('down');setLayerMenu(null);}}>Send backward</button>
      <button role="menuitem" onClick={()=>{toggleLock(selected);setLayerMenu(null);}}>{selected.lockMovementX?'Unlock':'Lock'} layer</button>
      {<><button role="menuitem" disabled={!!selected.lockMovementX} onClick={()=>{mergeDown();setLayerMenu(null);}}>Merge Down · Ctrl E</button><button role="menuitem" disabled={!!selected.lockMovementX} onClick={()=>{openWarp();setLayerMenu(null);}}>Transform · Warp</button><button role="menuitem" disabled={!!selected.lockMovementX} onClick={()=>{setTool('eraser');setLayerMenu(null);}}>Soft eraser</button></>}
      <button role="menuitem" disabled={!!selected.lockMovementX} onClick={()=>{removeSelected();setLayerMenu(null);}}>Delete</button><button role="menuitem" onClick={()=>setLayerMenu(null)}>Close</button>
    </div></>}
