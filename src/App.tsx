...
type Obj = FabricObject & { textSoftness?:number; pdfFontData?:string;pdfOriginalFont?:string;pdfFontFallback?:string;recognizedKey?:string; id?: string; name?: string; diagramXml?: string; text?: string; fontSize?: number; fontFamily?: string; fontWeight?: string | number; fontStyle?: string; underline?: boolean; textAlign?: string; isEditing?: boolean };
...
export default function App() {
  ...
  const [replacementBackground,setReplacementBackground]=useState<'repair'|'sampled'|'solid'|'transparent'>('repair'),[replacementColor,setReplacementColor]=useState('#ffffff');
  const [replacementPadding,setReplacementPadding]=useState(2);
  ...
  async function editDetectedText(region:TextRegion){
    ...
    try{if(region.kind==='ocr'&&replacementBackground==='transparent'&&originalPage.source)throw new Error('Unlock Original PDF first to clear scanned text to transparency.');let family=region.kind==='ocr'?ocrFont:fallbackFont(region.fontName),embedded=false;if(region.fontData&&region.fontId){try{await ensureEmbeddedFont(region.fontId,region.fontData);family=region.fontId;embedded=true;}catch{}}await ensureFont(family);
      ...
      if(region.kind==='ocr'){
        const padding=replacementPadding,left=Math.max(0,region.left-padding),top=Math.max(0,region.top-padding),width=Math.min(originalPage.width,region.left+region.width+padding)-left,height=Math.min(originalPage.height,region.top+region.height+padding)-top;
