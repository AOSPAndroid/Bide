...
export default function App() {
  ...
  useEffect(()=>{
    if(!ready||initializing||loading.current)return;let cancelled=false;setTextRegions([]);setTextScanStatus('');const p=page,c=canvas.current!;
    ...
    const scan=async()=>{setTextScanStatus('Detecting text…');try{
      ...
      if(p.source){const image=new Image();image.src=await pagePreview(p.source,p.index,2);await image.decode();source=document.createElement('canvas');source.width=image.width;source.height=image.height;source.getContext('2d')!.drawImage(image,0,0);}else source=pageRaster(c,p.width,p.height,c.getObjects().filter(o=>o instanceof FabricImage));
      ...
      const {recognizeImage}=await import('./browser/ocr');const detected=await recognizeImage(reduced);if(cancelled)return;const sx=p.width/reduced.width,sy=p.height/reduced.height;const regions=detected.map(r=>({...r,left:r.left*sx,top:r.top*sy,width:r.width*sx,height:r.height*sy,size:r.size*sy,baseline:r.baseline*sy})).filter(r=>!c.getObjects().some(o=>(o as Obj).recognizedKey===recognitionKey(r)));
      setTextRegions(regions);setTextScanStatus(regions.length?`${regions.length} OCR regions · English · suggested fonts`:c.getObjects().some(o=>(o as Obj).recognizedKey)?'Original text converted. Double-click the text layer to edit.':'No text detected. Use Text to add your own.');
    }catch(e){if(!cancelled)setTextScanStatus('Text detection unavailable: '+(e instanceof Error?e.message:String(e)));}};void scan();return()=>{cancelled=true;};
  ...
  async function editDetectedText(region:TextRegion){
    ...
    try{let family=fallbackFont(region.fontName),embedded=false;if(region.fontData&&region.fontId){try{await ensureEmbeddedFont(region.fontId,region.fontData);family=region.fontId;embedded=true;}catch{}}await ensureFont(family);
      const text=Object.assign(new Textbox(region.text,{left:region.left,top:region.top,width:Math.max(region.width+4,20),fontSize:region.size,fontFamily:family,fontWeight:embedded?'normal':region.bold?'bold':'normal',fontStyle:embedded?'normal':region.italic?'italic':'normal',fill:region.color,lineHeight:1,splitByGrapheme:true}),{id:uid(),name:region.text.slice(0,40),pdfOriginalFont:region.fontName,pdfFontData:embedded?region.fontData:undefined,pdfFontFallback:fallbackFont(region.fontName),recognizedKey:recognitionKey(region)});
      // Fabric's top includes its ascender allowance; anchor the first baseline.
      text.top=region.kind==='pdf'?region.baseline-region.size*.93:region.top-region.size*.18;
