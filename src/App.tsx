...
const TOOLS: { id: Tool; label: string; key: string; icon: typeof Type }[] = [
  ...
  { id: 'highlight', label: 'Highlight', key: 'H', icon: Highlighter }, { id: 'eraser', label: 'Eraser', key: 'E', icon: Eraser }, { id:'marquee',label:'Rectangular selection',key:'M',icon:SquareDashed }, { id:'lasso',label:'Lasso selection',key:'Q',icon:LassoSelect }, { id: 'hand', label: 'Pan', key: 'Space', icon: Hand },
];
...
export default function App() {
  ...
  useEffect(() => {
    ...
    c.on('mouse:down', e => {
      ...
      if(t==='marquee'||t==='lasso'){
        const target=c.getObjects().find(o=>(o as Obj).id===selectionTarget.current);
  ...
  useEffect(() => {
    ...
    setRegion(null);selectionTarget.current=null;
  ...
  async function applySelection(mode:'keep'|'hide'|'copy') {
    ...
    try {
      ...
      if(mode==='copy'){
        setBusy('Creating cutout layer…');const copy=await object.clone() as Obj;
        copy.set({clipPath:mask});Object.assign(copy,{id:uid(),name:`${object.name||'Layer'} cutout`});c.add(copy);c.setActiveObject(copy);
  ...
  const actions = [
    ...
    {label:'Freehand lasso selection',hint:'Q',icon:LassoSelect,run:()=>setTool('lasso')},
  ...
  useEffect(() => {
    const key = (e: KeyboardEvent) => {
      ...
      if(e.key==='Escape') { setMobilePanel(null); setRegion(null); canvas.current?.discardActiveObject(); canvas.current?.requestRenderAll(); setTool('select'); setMenu(null); }
  ...
  return <div className={`app workspace-${view}`} onDragOver={e=>{ if(view!=='diagrams'&&e.dataTransfer.types.includes('Files')) { e.preventDefault(); setDraggingFile(true); } }} onDrop={e=>{ if(view!=='diagrams'&&e.dataTransfer.files.length) { e.preventDefault(); setDraggingFile(false); setPdfHome(false);void importFiles([...e.dataTransfer.files],true); } }}>
    ...
    <main className="workspace-layout" data-panel={mobilePanel||undefined}><aside className="tool-rail" aria-label="Drawing tools">
      {([{name:'Select',ids:['select','marquee','lasso','hand']},{name:'Create',ids:['text','rect','ellipse','line']},{name:'Draw',ids:['brush','highlight','eraser']}] as {name:string;ids:Tool[]}[]).map(group=><div className="tool-group" key={group.name}><h3>{group.name}</h3><div>{group.ids.map(id=>{const t=TOOLS.find(item=>item.id===id)!;return <button key={id} title={`${t.label} (${t.key})`} aria-label={t.label} aria-pressed={tool===id} className={tool===id?'active':''} onClick={()=>setTool(id)}><t.icon size={18}/><span>{({select:'Move',marquee:'Marquee',lasso:'Lasso',hand:'Pan',text:'Text',rect:'Rectangle',ellipse:'Ellipse',line:'Line',brush:'Brush',highlight:'Highlight',eraser:'Eraser'} as Record<Tool,string>)[id]}</span><kbd>{t.key==='Space'?'Space':t.key}</kbd></button>;})}</div></div>)}
